...
public final class CubicHermiteSpline extends Spline {
    ...
    public CubicHermiteSpline(final RigidTransform p0, final RigidTransform p1) {
        final double x0, x1, dx0, dx1, y0, y1, dy0, dy1;
        final double scale = 2 * p0.getTranslation().distance(p1.getTranslation());
    ...
    public Rotation getHeading(double t) {
        ...
        return new Rotation(dx, dy, true);
    }
    ...
    public double getCurvature(double t) {
        ...
        return (dx * ddy - dy * ddx) / ((dx * dx + dy * dy) * sqrt(dx * dx + dy * dy));
    }
