...
public class QuinticHermiteSpline extends Spline {
     ...
     */
    public QuinticHermiteSpline(final RigidTransform p0, final RigidTransform p1) {
        final double scale = 1.2 * p0.getTranslation().distance(p1.getTranslation());
    ...
    private QuinticHermiteSpline(
    		final double x0, final double x1, final double dx0, final double dx1,
			final double ddx0, final double ddx1, final double y0, final double y1,
		 	final double dy0, final double dy1, final double ddy0, final double ddy1
    ) {
        this.x0 = x0;
        this.x1 = x1;
        this.dx0 = dx0;
    ...
    public RigidTransform getStartPose() {
        return new RigidTransform(
                ...
                new Rotation(dx0, dy0, true)
        );
    }

    public RigidTransform getEndPose() {
        return new RigidTransform(
                new Translation(x1, y1),
                new Rotation(dx1, dy1, true)
        );
    }
     ...
     */
    @Override
    public Translation getPoint(final double t) {
    ...
    private double dddx(final double t) {
        return 60 * ax * t * t + 24 * bx * t + 6 * cx;
    }

    private double dddy(final double t) {
    ...
    @Override
    public double getDCurvature(final double t) {
        final var dx2dy2 = (dx(t) * dx(t) + dy(t) * dy(t));
        final var num = (dx(t) * dddy(t) - dddx(t) * dy(t)) * dx2dy2 - 3 * (dx(t) * ddy(t) - ddx(t) * dy(t)) * (dx(t) * ddx(t) + dy(t) * ddy(t));

        return num / (dx2dy2 * dx2dy2 * sqrt(dx2dy2));
    }

    private double dCurvature2(final double t) {
        final var dx2dy2 = (dx(t) * dx(t) + dy(t) * dy(t));
        final var num = (dx(t) * dddy(t) - dddx(t) * dy(t)) * dx2dy2 - 3 * (dx(t) * ddy(t) - ddx(t) * dy(t)) * (dx(t) * ddx(t) + dy(t) * ddy(t));
    ...
    public Rotation getHeading(double t) {
        return new Rotation(dx(t), dy(t), true);
    }
     ...
     */
    public static double sumDCurvature2(final List<QuinticHermiteSpline> splines) {
        double sum = 0;
     ...
     */
    public static double optimizeSpline(final List<QuinticHermiteSpline> splines) {
        int count = 0;
    ...
    private static void runOptimizationIteration(List<QuinticHermiteSpline> splines) {
        ...
        final ControlPoint[] controlPoints = new ControlPoint[splines.size() - 1];
        double magnitude = 0;

        for (int i = 0; i < splines.size() - 1; ++i) {
            // don't try to optimize colinear points
            if (splines.get(i).getStartPose().isCollinear(splines.get(i + 1).getStartPose()) || splines.get(i).getEndPose().isCollinear(splines.get(i + 1).getEndPose())) {
                continue;
            }

            final double original = sumDCurvature2(splines);
